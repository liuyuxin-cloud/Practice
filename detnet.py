...
def det_net(features, num_resnet_blocks, num_resnet_features,
  ...
  with tf.variable_scope(scope, 'DetNet'):
    ...
    with tf.name_scope('seg'):
        seg_logits = Conv2D(2, (1, 1), activation=None, kernel_initializer='random_normal', bias_initializer='zeros',
                            name='logits')(x)
        seg_preds = Softmax()(seg_logits)

    # Regression prediction
    with tf.name_scope('reg'):
        reg_preds = Conv2D(4, (1, 1), activation='relu', kernel_initializer='random_normal', bias_initializer='zeros',
                           name='reg_preds')(x)
...
def det_net_loss(seg_masks_in, reg_masks_in,
  ...
  with tf.variable_scope('loss'):
    ...
    # segmentation loss
    seg_masks_onehot = tf.one_hot(seg_masks_in_ds[:, :, :, 0], depth=2)
