...
with tf.device('/gpu:0'):
    ...
    with tf.variable_scope('ActNet'):
        ...
        with tf.variable_scope('sequence'):
            ...
            boxes_hidden_flat = tf.keras.layers.Dense(NFH, activation='tanh')(boxes_features_flat_dropout)
            ...
            def _construct_sequence(batch):
                ...
                for t in range(1, T):
                    ...
                    def _construct_update(reuse):
                        ...
                        reset = tf.keras.layers.Dense(NFH, activation='sigmoid', name='reset')(state)

                        # update gate
                        step = tf.keras.layers.Dense(NFH, activation='sigmoid', name='step')(state)

                        # gated state
                        state_r = tf.concat([reset * state_prev, hidden], axis=1)

                        # updated state
                        state_up = tf.keras.layers.Dense(NFH, activation='tanh', name='state_up')(state_r)
            ...
            with tf.variable_scope('actions_hidden'):
                ...
                actions_logits = tf.keras.layers.Dense(c.num_actions, activation=None)(boxes_states_flat)

                # Actions predictions (softmax)
                actions_preds = tf.nn.softmax(actions_logits, axis=-1)

                # Actions in one-hot encoding
                actions_in_one_hot = tf.one_hot(actions_in_flat, c.num_actions)
            ...
            with tf.variable_scope('activities_hidden'):
                ...
                activities_logits = tf.keras.layers.Dense(c.num_activities, activation=None)(boxes_states_pooled_flat)

                # Activities labels (argmax)
                activities_labels = tf.argmax(activities_logits, axis=1)

                # Activities accuracy
                activities_accuracy = tf.reduce_mean(
                    tf.cast(tf.equal(tf.cast(activities_labels, tf.int32), activities_in_flat), tf.float32))

                # Activities predictions (softmax)
                activities_preds = tf.nn.softmax(activities_logits, axis=-1)

                # Activities in one-hot encoding
                activities_in_one_hot = tf.one_hot(activities_in_flat, c.num_activities)
