...
VAL_SEQS = [0, 2, 12, 17, 19, 24, 26, 27, 28, 30, 33, 46, 49, 51]
TEST_SEQS = [4, 5, 11, 14, 20, 21, 25, 29, 34, 35, 37, 43, 44, 45, 47]
...
def volley_read_dataset(path, seqs):
    data = {}
    for sid in seqs: # 第sid个视频段
        data[sid] = volley_read_annotations(path + '/%d/annotations.txt' % sid)
    return data # 返回数据集中所有注释的字典


def volley_all_frames(data):
    frames = []
    for sid, anns in data.items():
        for fid, ann in anns.items(): # 对于每一个注释文件，读取每一条记录
            frames.append((sid, fid)) # （视频号，关键帧号）
    return frames
...
def volley_frames_around(frame, num_before=5, num_after=4):
    sid, src_fid = frame
    return [(sid, src_fid, fid) # （视频号，关键帧号，前/后帧号）
